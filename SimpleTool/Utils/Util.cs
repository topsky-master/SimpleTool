...
#pragma warning restore IDE0060 // Remove unused parameter
	...
	ForgeTypeId u)
		...
		static string _caption = Constants.BRAND;
...
#pragma warning restore IDE0051 // Remove unused private members
		...
		public static void GetDimensionOfOpening(Document doc, COpening pOpening)
		{
			Element opening = doc.GetElement(pOpening.Id);
			...
			{
				FamilyInstance inst = opening as FamilyInstance;
				HostObject hostObject = doc.GetElement(inst.Host.Id) as HostObject;
				List<Face> baseFaces = GetBaseFacesOfElement(doc, hostObject);
				//inst.Category.CategoryType == CategoryType.Model

				List<PlanarFace> faceList = GetOpeningPlanarFaces(hostObject, pOpening.Id);
							...
							if (c is Line)
							{
								foreach (Face f in baseFaces)
								{
									if (f is PlanarFace)
									{
										...
										if (IsZero(plane.SignedDistanceTo(c.GetEndPoint(0)))
											&& IsZero(plane.SignedDistanceTo(c.GetEndPoint(1))))
										{
											XYZ ptStart = c.GetEndPoint(0), ptEnd = c.GetEndPoint(1);
											for (i = 0; i < pts.Count; i++)
											{
												if (IsEqual(pts[i], ptStart))
													break;
											}
											if (i == pts.Count)
												pts.Add(ptStart);
			...
			else if (opening is Opening)
			{
				...
				ICollection<ElementId> eIds = doc.Delete(opening.Id);
				trans.RollBack();
				...
				foreach (ElementId eId in eIds)
				{
					if (doc.GetElement(eId) is ModelLine)
					{
						...
						XYZ ptStart = ml.GeometryCurve.GetEndPoint(0)
							, ptEnd = ml.GeometryCurve.GetEndPoint(1);
						for (i = 0; i < pts.Count; i++)
						{
							if (IsEqual(pts[i], ptStart))
								break;
			...
			pOpening.Points = new XYZ[pts.Count];
			for (i = 0; i < pts.Count; i++)
			{
				pOpening.Points[i] = pts[i];
			}

			/*fWidth = 0;
			fHeight = 0;
			Parameter para = f.LookupParameter("Rough Width");
			if (para != null && para.HasValue)
			{
				fWidth = para.AsDouble();
			}
			else
			{
				para = f.LookupParameter("Width");
				if (para != null && para.HasValue)
				{
					fWidth = para.AsDouble();
				}
				...
				{
					Element family = doc.GetElement(f.GetTypeId());
					para = family.LookupParameter("Rough Width");
					if (para != null && para.HasValue)
					{
						fWidth = para.AsDouble();
					}
					...
					{
						para = family.LookupParameter("Width");
						if (para != null && para.HasValue)
						{
							fWidth = para.AsDouble();
						}
						...
						{
							WinForms.MessageBox.Show("Can't get width and height property of the Opening. This opening won't be calculated. Please check if the opening has \"Rough Width\" and \"Rough Height\" property");
							//throw new Exception("Can't get width and height property.");
						}
					}
				}
			}

			f.LookupParameter("Rough Height");
			if (para != null && para.HasValue)
			{
				fHeight = para.AsDouble();
			}
			else
			{
				para = f.LookupParameter("Height");
				if (para != null && para.HasValue)
				{
					fHeight = para.AsDouble();
				}
				else
				{
					...
					{
						Element family = doc.GetElement(f.GetTypeId());
						para = family.LookupParameter("Rough Height");
						if (para != null && para.HasValue)
						{
							fHeight = para.AsDouble();
						}
						...
						{
							para = family.LookupParameter("Length");
							if (para != null && para.HasValue)
							{
								fHeight = para.AsDouble();
							}
		...
		/// <summary>
		/// This function works when both elements are slabs and walls
		/// </summary>
		/// <param name="e1"></param>
		/// <param name="e2"></param>
		/// <returns></returns>
		public static SetComparisonResult IntersectWith(Element e1, Element e2)
		{
			...
			foreach (GeometryObject geomObject1 in geomElem1)
			{
				...
				{
					Solid solid1 = geomObject1 as Solid;
					FaceArray faceArray1 = solid1.Faces;
					foreach (Face f1 in faceArray1)
					{
						foreach (GeometryObject geomObject2 in geomElem2)
						{
							if (geomObject2 is Solid)
							{
								Solid solid2 = geomObject2 as Solid;
								FaceArray faceArray2 = solid2.Faces;
								foreach (Face f2 in faceArray2)
								{
									SetComparisonResult scr = IntersectWith(f1, f2);
									if (scr == SetComparisonResult.Superset
										|| scr == SetComparisonResult.Subset
										|| scr == SetComparisonResult.Overlap)
									{
										return SetComparisonResult.Superset;
									}
								}
							}
						}
		...
		public static SetComparisonResult IntersectWith(Face f1, Face f2)
		{
			...
			{
				Plane p = f1.GetSurface() as Plane;
				List<XYZ> points1 = GetBoundaryPoints(f2);
				int nPlus = 0, nMinus = 0, i;
				for (i = 0; i < points1.Count; i++)
				{
					if (p.SignedDistanceTo(points1[i]) >= 0)
						nPlus++;
					else
						nMinus++;
				...
				p = f2.GetSurface() as Plane;
				List<XYZ> points2 = GetBoundaryPoints(f1);
				nPlus = 0; nMinus = 0;
				for (i = 0; i < points2.Count; i++)
				{
					if (p.SignedDistanceTo(points2[i]) >= 0)
						nPlus++;
					else
						nMinus++;
				...
				for (i = 0; i < points1.Count; i++)
				{
					if (IsPointInsideCurveLoop(points2, p.ProjectOnto(points1[i])))
					{
						return SetComparisonResult.Overlap;
					}
				}
				p = f1.GetSurface() as Plane;
				for (i = 0; i < points2.Count; i++)
				{
					if (IsPointInsideCurveLoop(points1, p.ProjectOnto(points2[i])))
					{
						return SetComparisonResult.Overlap;
					}
				}
			...
			if (IsZero((f1.GetSurface() as Plane).SignedDistanceTo(edges2[0].GetEndPoint(0))))
			{
				...
				{
					if (IsPointInsideCurveLoop(edges1, c.GetEndPoint(0)))
					{
						nInCount++;
					}
				}
				if (nInCount == edges2.Count)
				{
					return SetComparisonResult.Superset;
				}
				else if (nInCount == 0)
		...
		/// This function works only if the point is on the plane defined by the curve loop
		/// </summary>
		/// <param name="cl"></param>
		/// <param name="pt"></param>
		/// <returns></returns>
		public static bool IsPointInsideCurveLoop(CurveLoop cl, XYZ pt, bool bIncludeBoundary = true)
		{
			Plane plane;
			foreach (Curve c in cl)
			{
				...
				{
					//	Not supported
					LogManager.Write(LogManager.WarningLevel.Error, "Util::IsPointInsideCurveLoop", "This kind of curve loop is not supported.");
					return false;
				}
			}
			...
			{
				if (nIndex == 0)
					pts[nIndex++] = c.GetEndPoint(0);
				else
				{
					if (c.GetEndPoint(0).DistanceTo(pts[nIndex - 1]) > 0.005)
						pts[nIndex++] = c.GetEndPoint(0);
				}
				if (nIndex > 2)
					break;
			...
			if (!Util.IsEqual(pt, plane.ProjectOnto(pt)))
				return false;

			pt = plane.ProjectOnto(pt);

			//	Check the boundary edges first.
			foreach (Curve c in cl)
			{
				XYZ pt1 = c.GetEndPoint(0);
				XYZ pt2 = c.GetEndPoint(1);

				if (Util.IsZero(c.Distance(pt))
					&& Util.IsEqual(c.Length, pt.DistanceTo(pt1) + pt.DistanceTo(pt2))
					)
				{
			...
			List<UV> uvArray = new List<UV>();
			foreach (Curve c in cl)
			{
				plane.Project(c.GetEndPoint(0), out UV uv, out double _);
				uvArray.Add(uv);
			}
			PointInPoly pip = new PointInPoly();
			plane.Project(pt, out UV uv1, out double _);
			return pip.PolygonContains(uvArray, uv1);
		}

		public static bool IsPointInsideCurveLoop(List<Curve> curves, XYZ pt, bool bIncludeBoundary = true)
		{
			CurveLoop cl = new CurveLoop();
			...
			return IsPointInsideCurveLoop(cl, pt, bIncludeBoundary);
		}

		public static bool IsPointInsideCurveLoop(XYZ[] pts, XYZ pt, bool bIncludeBoundary = true)
		{
			CurveLoop cl = new CurveLoop();
			for (int i = 0; i < pts.Length - 1; i++)
			{
				cl.Append(Line.CreateBound(pts[i], pts[i + 1]));
			}
			cl.Append(Line.CreateBound(pts[pts.Length - 1], pts[0]));
			return IsPointInsideCurveLoop(cl, pt, bIncludeBoundary);
		}

		public static bool IsPointInsideCurveLoop(List<XYZ> pts, XYZ pt, bool bIncludeBoundary = true)
		{
			CurveLoop cl = new CurveLoop();
			...
			cl.Append(Line.CreateBound(pts[pts.Count - 1], pts[0]));
			return IsPointInsideCurveLoop(cl, pt, bIncludeBoundary);
		}

		/// <summary>
		/// Judge whether an element has face geometry and return those faces
		/// </summary>
		...
		public static bool GetFaceList(Element elem, out List<PlanarFace> faceList, bool bComputeReferences = false)
		{
			...
			InquireGeometry(geoElement, elem, out faceList);
			return true;
		}
